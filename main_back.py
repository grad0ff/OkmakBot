...
import appс
bot = telebot.TeleBot(okmakbot_token)
...
@bot.message_handler(regexp='Все продукты')
...
def handle(call):
    ...
    if call.data not in shopping_list.shoplist:
    ...
    #     bot.answer_callback_query(callback_query_id= call.id, text=f'Удалено из перечня:   {str(call.data)}')
