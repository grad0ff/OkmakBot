...
async def selected_action(message: Union[Message, CallbackQuery], state: FSMContext):
    ...
    if isinstance(message, CallbackQuery):
        ...
        await state.update_data(action_obj=message)
        msg_txt = message.text
    # items_list = []
    # msg = ''
    ...
    if msg_txt == btn_show.text:
        ...
        if priority_1_items or priority_2_items:
            await message.answer(f'Ред. в {db_table.updated_time}')
        else:
...
async def get_mrkp(items_list, prefix=''):
    markup = InlineKeyboardMarkup()
